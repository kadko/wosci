<?php
/*
  $Id: manufacturers.php.tortoise.removed,v 1.1 2008/12/26 13:10:36 kako Exp $
...
// Display a drop-down
     ...
     // $info_box_contents = array();
      $info_box_contents =  tep_draw_form('manufacturers', tep_href_link(FILENAME_DEFAULT, '', 'NONSSL', false), 'get').tep_draw_pull_down_menu('manufacturers_id', $manufacturers_array, (isset($HTTP_GET_VARS['manufacturers_id']) ? $HTTP_GET_VARS['manufacturers_id'] : ''), 'onChange="this.form.submit();" size="' . MAX_MANUFACTURERS_LIST . '" style="width: 100%"') . tep_hide_session_id();
    }

for($i=1;$i<count($manufacturers_array);$i++){
$markalar .= '<div><a href="index.php?manufacturers_id='.$manufacturers_array[$i]['id'].'">'.$manufacturers_array[$i]['text'].'</a></div>';
