...
// Set the local configuration parameters - mainly for developers
  if (file_exists('includes1/local/configure.php')) include('includes1/local/configure.php');
...
// start the session
	define('SESSION_FORCE_COOKIE_USE2' , True);
  $session_started = false;
  if (SESSION_FORCE_COOKIE_USE2 == 'True') {
...
// redirect the customer to a friendly cookie-must-be-enabled page if cookies are disabled
    ...
    switch ($_GET['action']) {
      ...
      case 'buy_now' :        if (isset($_GET['products_id'])) {
                                $cart->add_cart($_GET['products_id'], $cart->get_quantity($_GET['products_id'])+1);//wosci 058
...
// include the password crypto functions
  require('functions/password_funcs.php');
