...
// TABLES: tax_rates, zones_to_geo_zones
  function tep_get_tax_rate($class_id, $country_id = -1, $zone_id = -1) {
	...
	$cdaid= get_user_meta($current_user->ID, 'customer_default_address_id');
        $check_country_query = tep_db_query("select entry_country_id, entry_zone_id from " . TABLE_ADDRESS_BOOK . " where customers_id = '" . (int)$current_user->ID. "' and address_book_id = '" . (int)$cdaid . "'");
        $check_country = tep_db_fetch_array($check_country_query);

        $country_id = $check_country['entry_country_id'];
        $zone_id = $check_country['entry_zone_id'];
      }
    }
...
// Return a random value
  ...
  function tep_count_customer_address_book_entries($id = '', $check_session = true) {
    ...
    if ($check_session == true) {
      if ( ($current_user->ID === 0) || ($id != $current_user->ID) ) {
        return 0;
      }
    }
  ...
  function customer_address_book_entry_ids($id = '', $check_session = true) {
    global $current_user;
...
// nl2br() prior PHP 4.2.0 did not convert linefeeds on all OSs (it only converted \n)
  ...
  function locateIp($ip){
    ...
    return array('latitude' => $latitude, 'longitude' => $longitude, 'zippostalcode' => $zippostalcode, 'city' => $city, 'region_name' => $region_name, 'country_name' => $country_name, 'country_code' => $country_code, 'ip' => $ip, 'region_code' => $region_code);

  }
  
function osC_Country_Id($code)
		{
  		$counry_query = tep_db_query("select countries_id from " . TABLE_COUNTRIES . " where countries_iso_code_2 = '" . $code . "'");
		$counry = tep_db_fetch_array($counry_query);
		return $counry['countries_id'];
		}
...
if (!function_exists('tep_class_exists')) {
  ...
  if (!function_exists('tep_cfg_select_option')) {
    ...
    if (!function_exists('tep_call_function')) {
  ...
  function tep_cfg_get_zone_name($zone_id) {
    ...
    return tep_draw_pull_down_menu('configuration_value', tep_get_countries2(), $country_id);
  }  
...
// preset the width of the drop-down for Netscape
    ...
    if (sizeof($zones) > 0) {
      $zones_select = array(array('id' => '', 'text' => __('All Zones','wosci-translation')));
      $zones = array_merge($zones_select, $zones);
    } else {
      $zones = array(array('id' => '', 'text' => __('All Zones','wosci-translation')));
// create dummy options for Netscape to preset the height of the drop-down
    ...
    while ($countries = tep_db_fetch_array($countries_query)) {
      ...
      while ($states = tep_db_fetch_array($states_query)) {
        if ($num_state == '1') $output_string .= '    ' . $form . '.' . $field . '.options[0] = new Option("' . __('Please Select','wosci-translation') . '", "");' . "\n";
    ...
    $output_string .= '  } else {' . "\n" .
                      '    ' . $form . '.' . $field . '.options[0] = new Option("' . __('All Zones','wosci-translation') . '", "");' . "\n" .
                      '  }' . "\n";
