...
*/
  ...
  class order {
    ...
    function query($order_id) {
      ...
      $index = 0;
      $orders_products_query = tep_db_query("select orders_products_id, products_id, products_name, products_model, products_price, products_tax, products_quantity, final_price, products_currency, currency_value from " . TABLE_ORDERS_PRODUCTS . " where orders_id = '" . (int)$order_id . "'");
      while ($orders_products = tep_db_fetch_array($orders_products_query)) {
        $this->products[$index] = array('qty' => $orders_products['products_quantity'],
	                                'id' => $orders_products['products_id'],
                                        ...
                                        'currency' => $orders_products['products_currency'],
...
$phone = get_user_meta($current_user->ID, 'phone');

      $this->customer = array('firstname' => $first_name[0],
     ...
     // EOF Separate Pricing Per Customer
      ...
      $t = get_post_custom_values('Tax_', $products[$i]['id']);
        ...
        $this->products[$index] = array('qty' => $products[$i]['quantity'],
                                        ...
                                        'tax' => tep_get_tax_rate($t[0], $tax_address['entry_country_id'], $tax_address['entry_zone_id']),
                                        'tax_description' => tep_get_tax_description($t[0], $tax_address['entry_country_id'], $tax_address['entry_zone_id']),
                                        'price' => $products[$i]['price'],
                                        'currency' => $products[$i]['currency'],
                                        'currency_value' => $products[$i]['currency_value'],
...
// EOF Separate Pricing Per Customer attribute_groups mod
        ...
        $shown_price = $currencies->calculate_price($this->products[$index]['currency'], $this->products[$index]['final_price'], $this->products[$index]['tax'], $this->products[$index]['qty']);
