...
// replace products prices with those from customers_group table
	...
	$listing[$x]['products_price'] = $raw_pr_arr[0];;
	$listing[$x]['final_price'] = $raw_pr_arr[0];;
	}else{
	$listing[$x]['products_price'] = $pr['p'][0];
	$listing[$x]['final_price'] = $pr['p'][0];
	}
	$i++;
	}
        ...
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_PRICE':
            ...
            if (tep_not_null($listing[$x]['specials_new_products_price'])) {
              $lc_text = '&nbsp;<s>' .  $currencies->display_price($listing[$x]['products_currency'], $listing[$x]['products_price'], tep_get_tax_rate($listing[$x]['products_tax_class_id'])) . '</s>&nbsp;&nbsp;<span class="productSpecialPrice">' . $currencies->display_price($listing[$x]['products_currency'], $listing[$x]['specials_new_products_price'], tep_get_tax_rate($listing[$x]['products_tax_class_id'])) . '</span>&nbsp;';
            } else {
              $lc_text = '&nbsp;' . $currencies->display_price($listing[$x]['products_currency'], $listing[$x]['products_price'], tep_get_tax_rate($listing[$x]['products_tax_class_id'])) . '&nbsp;';
            }
            break;
          case 'PRODUCT_LIST_QUANTITY':
            $lc_align = 'right';
