...
// Stock Check
  ...
  if ( ( is_array($payment_modules->modules) && (sizeof($payment_modules->modules) > 1) && !is_object($$payment) ) || (is_object($$payment) && ($$payment->enabled == false)) ) {
	wp_redirect('shipping-payment?error_message=' . urlencode(ERROR_NO_PAYMENT_MODULE_SELECTED));
  }
...
// Update products_ordered (for bestsellers list)
    tep_db_query("update " . TABLE_PRODUCTS . " set products_ordered = products_ordered + " . sprintf('%d', $order->products[$i]['qty']) . " where products_id = '" . tep_get_prid($order->products[$i]['id']) . "'");
     ...
     $pord = get_post_meta($order->products[$i]['id'], 'products_ordered');
     update_post_meta($order->products[$i]['id'], 'products_ordered', $pord[0]+1);
    ...
    $sql_data_array = array('orders_id' => $insert_id, 
                            ...
                            'products_currency' => $order->products[$i]['currency'], 
                            'currency_value' => $currencies->get_value($order->products[$i]['currency']), 
...
//------insert customer choosen option eof ----
    ...
    $products_ordered .= $order->products[$i]['qty'] . ' x ' . $order->products[$i]['name'] . ' (' . $order->products[$i]['model'] . ') = ' . $currencies->display_price($order->products[$i]['currency'], $order->products[$i]['final_price'], $order->products[$i]['tax'], $order->products[$i]['qty']) . $products_ordered_attributes . "\n";
  }
...
// lets start with the email confirmation
  $email_order = STORE_NAME . "\n" . 
                 ...
                 EMAIL_TEXT_INVOICE_URL . ' ' . esc_url( home_url( '/' ) ).'account-history-info?order_id=' . $insert_id. "\n" .
  ...
  if ($order->content_type != 'virtual') {
    $email_order .= "\n" . EMAIL_TEXT_DELIVERY_ADDRESS . "\n" . 
                    EMAIL_SEPARATOR . "\n" .
                    tep_address_label($current_user->ID, $sendto, 0, '', "\n") . "\n";
  }

  $email_order .= "\n" . EMAIL_TEXT_BILLING_ADDRESS . "\n" .
                  EMAIL_SEPARATOR . "\n" .
                  tep_address_label($current_user->ID, $billto, 0, '', "\n") . "\n\n";
  ...
  wp_mail($order->customer['email_address'], __('Order Process', 'wosci-language'), $email_order);
